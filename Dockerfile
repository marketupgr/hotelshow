...
#   - Ex: hexpm/elixir:1.17.3-erlang-27.2-debian-bullseye-20250113-slim
#
...
RUN curl -fsSL https://bun.sh/install -o install_bun.sh && \
    bash install_bun.sh && \
    rm install_bun.sh
...
# compile assets
RUN PATH=$PATH:~/.bun/bin mix assets.deploy
...
COPY --from=builder /app/_build/${MIX_ENV}/rel/game ./
